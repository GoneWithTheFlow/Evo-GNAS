...
data_path = r"C:\Users\XinLiu\Desktop\QUBO\data\G49.txt"
...
class GEN_GNN_MODEL(torch.nn.Module):
    def __init__(self):
        ...
        self.gnn_2 = GCNConv(in_channels=64, out_channels=1)
        self.gnn_3 = ChebConv(in_channels=1, out_channels=29, K=4)
        self.gnn_4 = GCNConv(in_channels=29, out_channels=1)
        self.gnn_5 = ChebConv(in_channels=1, out_channels=1, K=4)

    def forward(self, x_0, edge_index):
        x_2 = F.elu(self.gnn_2(x_0, edge_index))
        x_2 = F.dropout(x_2, p=0.4, training=self.training)
        x_3 = F.elu(self.gnn_3(x_2, edge_index))
        x_3 = F.dropout(x_3, p=0.5, training=self.training)
        x_4 = F.elu(self.gnn_4(x_3, edge_index))
        x_4 = F.dropout(x_4, p=0.4, training=self.training)
        x_5 = F.elu(self.gnn_5(x_4, edge_index))
        return torch.sigmoid(x_5)
