...
data_path = r"C:\Users\XinLiu\Desktop\QUBO\data\G15.txt"
...
class GEN_GNN_MODEL(torch.nn.Module):
    def __init__(self):
        ...
        self.gnn_1 = GCNConv(in_channels=64, out_channels=122)
        self.gnn_2 = GCNConv(in_channels=64, out_channels=76)
        self.gnn_3 = GCNConv(in_channels=122, out_channels=76)
        self.gnn_5 = SAGEConv(in_channels=76, out_channels=1)
        self.gnn_6 = SAGEConv(in_channels=1, out_channels=1)

    def forward(self, x_0, edge_index):
        x_1 = self.gnn_1(x_0, edge_index)
        x_1 = F.dropout(x_1, p=0.4, training=self.training)
        x_2 = F.elu(self.gnn_2(x_0, edge_index))
        x_2 = F.dropout(x_2, p=0.5, training=self.training)
        x_3 = F.relu(self.gnn_3(x_1, edge_index))
        x_3 = F.dropout(x_3, p=0.4, training=self.training)
        x_4 = x_3 + x_2
        x_5 = self.gnn_5(x_4, edge_index)
        x_5 = F.dropout(x_5, p=0.0, training=self.training)
        x_6 = self.gnn_6(x_5, edge_index)
        return torch.sigmoid(x_6)
