...
# Database model for raw architecture genes.
class Main(Base):
    __tablename__ = "main"
...
# Database model for accuracy and cut-value records.
class Acc(Base):
    __tablename__ = 'acc'
    id = Column(Integer, primary_key=True)
    accuracy = Column(Float, nullable=False)
    cut = Column(Float, nullable=False)

    def __repr__(self):
        return f"Acc(id={self.id!r}, accuracy={self.accuracy!r}, cut={self.cut!r})"


# Database model for parentage and iteration metadata.
...
# Database model for evolutionary search status.
class Status(Base):
...
# Database model for saved result file paths.
class ResFile(Base):
...
# Database model for failed training logs.
class BadCode(Base):
...
# Database model for normalized gene strings.
...
# Database model for graph hashes and sequence metadata.
class Hash(Base):
    __tablename__ = "hash"
    id = Column(Integer, primary_key=True)
...
# Database model for worker task claims.
class TaskGet(Base):
...
# Database model for registered worker processes.
...
# Database model for failed differentiable-search code logs.
...
# Database model for differentiable-search result scores.
class DSResult(Base):
...
# Read or initialize the evolutionary search status.
