...
data_path = r"C:\Users\XinLiu\Desktop\QUBO\data\G55.txt"
...
class GEN_GNN_MODEL(torch.nn.Module):
    def __init__(self):
        ...
        self.gnn_1 = GATConv(in_channels=64, out_channels=1, heads=8,concat=False,dropout=0.6)
        self.gnn_2 = ChebConv(in_channels=1, out_channels=61, K=3)
        self.gnn_3 = ChebConv(in_channels=61, out_channels=1, K=3)

    def forward(self, x_0, edge_index):
        x_1 = self.gnn_1(x_0, edge_index)
        x_1 = F.dropout(x_1, p=0.4, training=self.training)
        x_2 = F.leaky_relu(self.gnn_2(x_1, edge_index))
        x_2 = F.dropout(x_2, p=0.4, training=self.training)
        x_3 = self.gnn_3(x_2, edge_index)
        return torch.sigmoid(x_3)
