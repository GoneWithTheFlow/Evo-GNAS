...
# Collect training and best-score statistics for one iteration.
def get_iteration_stats(engine, iteration: int):
    with Session(engine) as session:
        runtime_rows = session.execute(
            """
            select h.hash, avg(r.runtime) as avg_runtime
            from generation g
            join runtime r on g.id = r.id
            join hash h on g.id = h.id
            where g.iteration={}
            group by h.hash
            """.format(iteration)
        ).all()

        new_trained = len(runtime_rows)
        runtime_sum = sum(float(row[1]) for row in runtime_rows)

        total_unique_trained_row = session.execute(
            """
        ...
        total_unique_trained = int(total_unique_trained_row[0] or 0)

        best_row = session.execute(
            """
            select a.id, a.mis_size
            from generation g
            join acc a on g.id = a.id
            where g.iteration<={}
            order by a.mis_size desc, a.id asc
            limit 1
            """.format(iteration)
        ).first()
        ...
        prev_best_row = None
        if iteration > 0:
            prev_best_row = session.execute(
                """
                select max(a.mis_size)
                from generation g
                join acc a on g.id = a.id
                where g.iteration<{}
                """.format(iteration)
    ...
    best_id = int(best_row[0])
    best_mis = float(best_row[1])
    prev_best_mis = float(prev_best_row[0]) if prev_best_row and prev_best_row[0] is not None else float('-inf')
    is_new_best = best_mis > prev_best_mis

    return {
        'iteration': iteration,
        'new_trained': new_trained,
        'runtime_sum': runtime_sum,
        'best_id': best_id,
        'best_mis': best_mis,
        'is_new_best': is_new_best,
        'total_unique_trained': total_unique_trained,
    }


# Sum recorded runtime across training jobs.
def get_total_runtime(engine):
    with Session(engine) as session:
        row = session.execute(
            """
            select coalesce(sum(avg_runtime), 0)
            from (
                select h.hash, avg(r.runtime) as avg_runtime
                from runtime r
                join hash h on r.id = h.id
                group by h.hash
            ) t
            """
        ).first()
...
# Generate the next EA population from selected parents.
...
# Cross two parent genes into child genes.
...
# Mutate one gene to maintain population diversity.
...
# Submit and score all untrained genes for one iteration.
...
# Wait until all training workers finish pending jobs.
...
# Normalize a gene and generate its executable model code.
...
# Repair invalid gene parameters.
...
# Repair invalid gene input references.
...
# Repair incompatible feature/channel shapes in genes.
...
# Find a nearby divisor for channel-shape repair.
...
# Insert a gene unit and adjust downstream input references.
...
# Submit generated code for training and fetch its score.
def train_score_code(engine, id: int, code: str):
    with Session(engine) as session:
        ...
        sql = (
            'select accuracy,mis_size from acc where id in '
            '(select id from hash where hash=(select hash from hash where id={})) '
        ...
        if row is not None:
            ...
            if row is None:
                row = Acc(id=id, accuracy=accuracy, mis_size=mis_size)
                session.merge(row)
                session.commit()
                return
...
# Advance EA status after training/scoring.
...
# Run the main evolutionary algorithm loop.
def ea_loop(engine, status_dict):
    ...
    if status_dict['status'] == 0:
        ...
        if stats is not None:
            ...
            with open(log_file, 'a', encoding='utf-8') as f:
                f.write(
                    "[Initialization] Iteration {}: Current best ID={}, Score={:.6f}, Total={}, Runtime={:.2f}s{}\n".format(
                        stats['iteration'],
                        stats['best_id'],
                        stats['best_mis'],
                        stats['total_unique_trained'],
                        stats['runtime_sum'],
                        marker
                    )
                )
        ...
        if stats is not None:
            ...
            with open(log_file, 'a', encoding='utf-8') as f:
                f.write(
                    "Iteration {:>2}: New generated={:>3}, Total={:>4}, Best ID={:>5}, Score={:.6f}, Runtime={:>8.2f}s{}\n".format(
                        stats['iteration'],
                        stats['new_trained'],
                        stats['total_unique_trained'],
                        stats['best_id'],
    ...
    final_stats = get_iteration_stats(engine, iteration)
    total_runtime = get_total_runtime(engine)
    with open(log_file, 'a', encoding='utf-8') as f:
        f.write("\n" + "=" * 80 + "\n")
        f.write("Final Result\n")
        f.write("=" * 80 + "\n")
        if final_stats is not None:
            f.write("Global best architecture ID: {}\n".format(final_stats['best_id']))
            f.write("Global best score: {:.6f}\n".format(final_stats['best_mis']))
            f.write("Maximum independent set: {:.1f}\n".format(final_stats['best_mis']))
            f.write("Total trained architectures: {}\n".format(final_stats['total_unique_trained']))
        f.write("Total runtime: {:.2f}s ({:.2f}h)\n".format(total_runtime, total_runtime / 3600.0))
        f.write("=" * 80 + "\n")

    print("Evolution completed!")
    if final_stats is not None:
        print("Global best architecture ID={}, Score={:.6f}, Maximum independent set={:.1f}".format(
            final_stats['best_id'], final_stats['best_mis'], final_stats['best_mis']
        ))
    print("Total runtime: {:.2f}s ({:.2f}h)".format(total_runtime, total_runtime / 3600.0))
    print("Statistics saved to {}".format(log_file))
