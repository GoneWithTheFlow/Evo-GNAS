...
from init_ea import Status, Score, Acc, Task, TaskEnd, TaskGet, WorkerRegister
...
def _gene_hash(gene: str) -> str:
    ...
    seq, info = gene_graph_seq_with_info(units)
...
def select_generate_gene(engine, iteration):
    with Session(engine) as session:
        cfg: init_ea.Number = session.query(init_ea.Number).order_by(init_ea.Number.id.desc()).first()
