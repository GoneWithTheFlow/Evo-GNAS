...
# Extract the total trained architecture count from a log row.
def _extract_total_count(line):
    for token in ("Total=", "\u603b\u6570="):
...
def inspect_evolution_stats():
    ...
    total = _extract_total_count(iterations[-1])
    if total is not None:
        print(f"Total trained architectures: {total}")
