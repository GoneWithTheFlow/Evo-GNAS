...
data_path = r"C:\Users\XinLiu\Desktop\QUBO\data\G50.txt"
...
class GEN_GNN_MODEL(torch.nn.Module):
    def __init__(self):
        ...
        self.gnn_2 = GATConv(in_channels=64, out_channels=55, heads=7,concat=False,dropout=0.3)
        self.gnn_6 = ChebConv(in_channels=55, out_channels=111, K=4)
        self.gnn_7 = SAGEConv(in_channels=111, out_channels=1)
        self.gnn_8 = ChebConv(in_channels=1, out_channels=1, K=4)

    def forward(self, x_0, edge_index):
        x_2 = F.relu(self.gnn_2(x_0, edge_index))
        x_2 = F.dropout(x_2, p=0.3, training=self.training)
        x_6 = F.relu(self.gnn_6(x_2, edge_index))
        x_6 = F.dropout(x_6, p=0.4, training=self.training)
        x_7 = F.relu(self.gnn_7(x_6, edge_index))
        x_7 = F.dropout(x_7, p=0.0, training=self.training)
