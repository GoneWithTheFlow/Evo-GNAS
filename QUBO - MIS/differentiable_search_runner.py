...
from gnn_trainer import DATASET,Config
...
from architecture_codegen import gene_to_code
from gene_graph import gene_gene_graph_with_info
from database import Code, CodeFile, GeneExpressed, Hash, Length, Main, ResFile, Runtime, create_db_engine, Score, Acc,Generation
from gnn_trainer import train
...
from qubo_utils import loss_func as qubo_loss_func
from qubo_utils import create_Q_matrix
from qubo_utils import repair_to_independent_set_tensor
from qubo_utils import get_undirected_edge_pairs
from gnn_trainer import get_edge_index
...
# Differentiable-search cell over candidate GNN operators.
class GNNCell(nn.Module):
    ...
    def _build_gnn_layers(self, in_channels, hidden_dim, heads, dropout_prob):
        """Build all candidate GNN layers defined by get_gnn_params."""
        gnns = []
    ...
    def forward(self, x, edge_index):
        """Run the GNNCell forward pass with node features x and edge_index."""
    ...
    def get_weight_list(self):
        """Return the normalized weight list."""
        weight = F.softmax(self.weight, dim=0)
...
class DSModel(nn.Module):
    ...
    def forward(self, x, edge_index):
        """Run the GNN model forward pass with node features x and edge_index."""
        mid = {0: x}
        for i in range(1, len(self.ds_describe_items)):
            item = self.ds_describe_items[i]
            if item[0] == 1:
    ...
    def get_result_gene(self):
        """Generate the final gene sequence from selected GNN weights."""
        params = ["0"]
        for i in range(1, len(self.init_gene_params)):
            param = self.init_gene_params[i]
            mod = [i for i in param]
...
# Run differentiable search for one gene subspace.
...
from torch_geometric.nn import GCNConv, GATConv, SAGEConv, GATv2Conv, ChebConv, GINConv, ARMAConv'''
    ...
    final_model_dirname, best_mis_size, best_acc, runtime = train(model_class=model_class, save_dir=re_save_dir)

    new_score = best_acc
    print(f"best_mis_size: {best_mis_size}, best_acc: {best_acc}")
    ...
    with Session(engine) as session:
        ...
        session.merge(row)
        row = Acc(id=new_id, accuracy=best_acc, mis_size=best_mis_size)
        session.merge(row)
...
# Train DSModel and return the best discovered gene.
def ds_train(ds_describe, init_gene, ckpt_dir, save_dir, log_dir, epochs, prob_threshold=0.5,device=None):
    ...
    def scan_best_threshold(probs):
        ...
        def quick_qubo_score(bs):
            bs_row = bs.unsqueeze(0)
            return -(bs_row @ Q @ bs_row.T).item()

        for th in thresholds:
            bitstring = (probs.detach() >= th).float()
            candidate_pool.append((quick_qubo_score(bitstring), th.item(), bitstring))

        num_samples = getattr(Config, "num_samples", 256)
        samples = torch.bernoulli(probs.repeat(num_samples, 1)).float().to(device)
        for i in range(num_samples):
            sample_bs = samples[i]
            candidate_pool.append((quick_qubo_score(sample_bs), best_th, sample_bs))

        topk = min(getattr(Config, "repair_topk", 128), len(candidate_pool))
        top_candidates = sorted(candidate_pool, key=lambda x: x[0], reverse=True)[:topk]
        for _, th_value, candidate_bs in top_candidates:
            repaired_bs = repair_to_independent_set_tensor(candidate_bs, edge_u, edge_v, scores=probs)
            repaired_row = repaired_bs.unsqueeze(0)
            repaired_cut = -(repaired_row @ Q @ repaired_row.T).item()
    ...
    for epoch in range(epochs):
        ...
        if current_cut > best_cut:
            ...
            print(f"Updated best gene (MIS obj: {best_cut:.4f}) | Gene: {best_gene}")
        ...
        if epoch % 100 == 0:
            log_bitstring = repair_to_independent_set_tensor((probs.detach() >= 0.5).float(), edge_u, edge_v, scores=probs)
            mis_size = int(log_bitstring.sum().item())
            print(f"Epoch {epoch} | Loss: {loss.item():.6f} | Best MIS obj: {best_cut:.4f} | repaired MIS size: {mis_size}")

    print(f"Training finished | Best MIS obj: {best_cut:.4f}")

    with open(os.path.join(log_dir, "experiment.txt"), "a") as f:
        f.write(f"DSModel     MIS_obj={best_cut:.4f}\n")
