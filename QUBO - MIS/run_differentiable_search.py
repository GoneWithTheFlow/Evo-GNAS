...
parser = argparse.ArgumentParser(description="A worker for differentiable search after GNN training. "
                                             "The name and gpu can be the same as training workers.")
...
# Claim the next available task from the database.
...
# Resume an unfinished task previously claimed by this worker.
...
# Record a failed generated code task.
...
# Generate and execute a worker script for one task.
def do_task(engine, ds_describe: str, gpu: int, lucky_string: str):
    ...
    train_script = '''import differentiable_search_runner
...
save_dir = "{}"
log_dir = "{}"
new_id, new_score = differentiable_search_runner.search({}, iteration, ds_describe, init_id, init_gene,
...
'''.format(iteration, ds_describe, init_id, init_gene, init_score, ckpt_dir, save_dir, log_dir, gpu, epochs)
    ...
    script_path = './ds_script_gen_' + lucky_string + '.py'
